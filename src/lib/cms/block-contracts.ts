...
export type CmsBlockContract = {
  type: CmsBlockType;
  schemaVersion: 1;
  visualEditor: 'typed' | 'json';
  validateDraft: Refine;
};
...
export const CMS_BLOCK_CONTRACTS: Record<CmsBlockType, CmsBlockContract> = Object.fromEntries(
  CMS_BLOCK_TYPES.map((type) => [
    type,
    {
      type,
      schemaVersion: 1 as const,
      visualEditor: typedEditorBlocks.has(type) ? 'typed' : 'json',
      validateDraft: refinements[type],
    },
  ]),
) as Record<CmsBlockType, CmsBlockContract>;

export function getCmsBlockContract(type: CmsBlockType): CmsBlockContract {
  return CMS_BLOCK_CONTRACTS[type];
}
